...
export default function DoodleRoadGame() {
    ...
    const btnStyle = { fontSize: 16, borderRadius: 8, border: '1px solid rgba(0,0,0,0.12)', background: '#2f80ed', cursor: 'pointer' };
    ...
    useEffect(() => {
        stateRef.current = {
            ...
            isDrawing: false,
            path: [],
            car: { x: 150, y: HEIGHT / 2 - 50, w: 46, h: 30, finished: false },
            playing: false,
            playProgress: 0,
            playTotalLen: 0,
            playSpeed: 120,
        ...
        s.finish = { x: WIDTH - 150, y: 250, w: 44, h: 44 };
    }, []);
    ...
    function drawCar(ctx, cx, cy, angle = 0) { ctx.save(); ctx.translate(cx, cy); ctx.rotate(angle); ctx.fillStyle = '#ff4757'; roundRect(ctx, -23, -12, 46, 24, 6, true, true); ctx.fillStyle = '#cde7ff'; ctx.fillRect(-12, -8, 24, 10); ctx.fillStyle = '#000'; ctx.beginPath(); ctx.arc(-12, 12, 6, 0, Math.PI * 2); ctx.fill(); ctx.beginPath(); ctx.arc(12, 12, 6, 0, Math.PI * 2); ctx.fill(); ctx.restore(); }
    ...
    function loop(ts) {
        ...
        const g = ctx.createLinearGradient(0, 0, 0, HEIGHT); g.addColorStop(0, 'rgba(0, 0, 0, 0)'); g.addColorStop(1, 'rgba(0,0,0,0.04)'); ctx.fillStyle = g; ctx.fillRect(0, 0, WIDTH, HEIGHT);
        ...
        if (s.finish) {
            ...
            // Draw the finish line (opaque or semi-transparent)
            ctx.globalAlpha = 0.4; // slightly transparent
            ctx.fillStyle = '#fd0419ff';

            if (ctx.roundRect) {
                ctx.beginPath();
                ctx.roundRect(-sW /2*3, -sH / 2, sW, sH, radius);
                ctx.fill();
            } else {
                const x = -sW / 2;
                const y = -sH / 2;
                ...
                ctx.lineTo(x + sW - radius, y);
                ctx.quadraticCurveTo(x + sW, y, x + sW, y + radius);
                ctx.lineTo(x + sW, y + sH - radius);
                ctx.quadraticCurveTo(x + sW, y + sH, x + sW - radius, y + sH);
                ctx.lineTo(x + radius, y + sH);
                ctx.quadraticCurveTo(x, y + sH, x, y + sH - radius);
            ...
            // --- Finish line detection & stopping the car ---
            ...
            const finishLeft = f.x - sW / 2;
            const finishRight = f.x + sW / 2;
            const finishTop = f.y - sH / 2;
            const finishBottom = f.y + sH / 2;

            // Check overlap
            const overlaps = carRight > finishLeft &&
                carLeft < finishRight &&
                carBottom > finishTop &&
                carTop < finishBottom;
            ...
            if (overlaps) {
                ...
                if (car.vy > 0) {
                    car.y = finishTop - car.h / 2; // place car on top of finish line
                } else if (car.vy < 0) {
                    car.y = finishBottom + car.h / 2; // if moving upward
                }
                car.vx = 0; // stop horizontal motion if needed
                car.vy = 0; // stop vertical motion

                if (!car.finished) {
                    car.finished = true; // mark as finished
                    console.log("Car reached the finish line!");
                }
            }
        }
        ...
        if (mode === 'draw' || mode === 'cut') {
            if (s.path.length > 0) { ctx.beginPath(); ctx.lineWidth = 2; ctx.strokeStyle = 'rgba(0,0,0,1)'; ctx.moveTo(s.path[0].x, s.path[0].y); for (let p of s.path) ctx.lineTo(p.x, p.y); ctx.stroke(); }
